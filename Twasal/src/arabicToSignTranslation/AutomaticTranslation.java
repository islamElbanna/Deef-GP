...
public class AutomaticTranslation implements Translator_Interface{
	...
	private void parseAndrefineArabic(LexicalizedParser lp, String sent2) {
		...
		ArrayList<String> grammer = getGrammerLabels(arabicParser.grammer
				.toString());
		System.out
				.println("=======================NEW TREE========================================================");
	...
	private void refineArabicTree(Tree arabicTree, ArrayList<String> grammer) { // annotate
																				// the
																				// tree
																				// with
																				// grammar
																				...
																				// the
																				// tree
		if (arabicTree.children().length == 1
				&& !arabicTree.children()[0].isLeaf()
				&& arabicTree.children()[0].children()[0].isLeaf()) {
			arabicTree.setLabel(new nodeValue(arabicTree.label().value() + ","
					+ arabicTree.firstChild().label().value()));
		...
		if (arabicTree.children().length == 1
			...
			if (grammer.contains(arabicTree.children()[0].value())) {
				...
				if (!grammer.get(index - 1).equalsIgnoreCase("root"))
					arabicTree.setLabel(new nodeValue(value + ","
							+ grammer.get(index - 1)));
	...
	private void ApplySignRules(int index, Tree t) {
		if (t.label().value().contains("VP")) {
			...
			if (t.firstChild().label().value().contains("WH"))
				parseWasel(index, t);
			else if (t.firstChild().label().value().contains("IN")
					&& t.firstChild().label().value().contains("mark"))
				parseShart(t);
		} else if (t.label().value().contains("SBARQ")
				|| t.label().value().contains("SQ")) {
			if (t.firstChild().label().value().contains("WH"))
				parseQuestion(t);
	...
	private void parseVerbPhrase(Tree t) {
		...
		if (childrens[0].label().value().contains("PRT")) {
			if (childrens[0].firstChild().label().value()
					.equalsIgnoreCase("سوف")) {
				t.removeChild(0);
				String label = t.firstChild().label().value();
				t.firstChild().setLabel(
						new nodeValue("VBF"
								+ label.substring(3, label.length())));
				updateWordType(t.firstChild().firstChild().label().value(), t
		...
		for (int i = 0; i < childrens.length; i++) {
			if (childrens[i].label().value().contains("VBD")
					|| childrens[i].label().value().contains("VBP")
					|| childrens[i].label().value().contains("VBF")) {
